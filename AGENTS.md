...
- `openai_responses` **MUST** 使用 `POST /responses`、图片输入、strict JSON Schema structured output 和 `store=false`；`openai_chat_completions` **MUST** 使用 `POST /chat/completions`、图片输入、strict JSON Schema structured output，并省略 `store`。两种协议都 **MUST** 使用同一 configured/requested `model_id` 覆盖离线标注、QuerySpec 和 visual rerank 三阶段、同一总重试预算、本地校验、稳定错误分类和最小披露；成功响应仍须有 string `model`，但第三方返回的不同 model echo 不得被当作已验证远端身份、持久化或替换 requested `model_id`。
