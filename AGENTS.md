...
4. 所有复选框 **MUST** 以可复核路径、精确命令输出、哈希、测试报告或发布清单为依据；没有证据 **MUST NOT** 标记完成。当前仓库已有 R0-R4 实现与对应证据；后续修订（包括 D-052、D-053）、R5、发布和跨平台事实仍必须分别以新增证据为准，**MUST NOT** 由既有证据推定完成。
...
- 子代理委派原则: **MUST** 仅在上下文有明确关联时复用既往子代理, 其余情况委派新的子代理. **MUST** 在任务性质产生变化后, 积极评估任务难度并派发适合的子代理 **MUST NOT** 用低级子代理惯性处理高难度任务. **MUST** 在每次委派子代理时, 强制进行难度系数评估和旧子代理任务相关度系数评估, 给出子代理的委派(复用)理由
...
- MVP 的 provider 层 **MUST** 使用 protocol-neutral `OpenAIProvider`，并且每个 profile 的现有 `adapter` 字段只能取显式枚举 `openai_responses` 或 `openai_chat_completions`；每个枚举对应一个明确的 wire adapter/codec。**MUST NOT** 自动 fallback、自动切换协议、实现 Anthropic/其他 provider adapter 或模型投票。
- `openai_responses` **MUST** 使用 `POST /responses`、图片输入、strict JSON Schema structured output 和 `store=false`；`openai_chat_completions` **MUST** 使用 `POST /chat/completions`、图片输入、strict JSON Schema structured output，并省略 `store`。两种协议都 **MUST** 使用同一 configured/requested `model_id` 覆盖 Studio 的 `offline_annotation`、同一总重试预算、本地校验、稳定错误分类和最小披露；既有 QuerySpec/rerank provider Schema 与历史 lineage 保留，但不构成 MCP runtime 能力。成功响应仍须有 string `model`，但第三方返回的不同 model echo 不得被当作已验证远端身份、持久化或替换 requested `model_id`。
- `base_url` 是所选协议 adapter 的用户批准 endpoint 配置，不是协议 fallback。任何协议都不能证明远端保留策略；**MUST NOT** 声称已验证远端存储/不存储，第三方服务信任与策略由用户负责。
...
- 允许保存多个非活动 provider profile 便于切换，但全局任一时刻最多一个 active profile；该约束仅适用于 Studio 新任务、配置管理和构建新 release。每个 release **MUST** 冻结其离线标注时的完整非秘密 provider snapshot，包括现有 `adapter`、`profile_id`、requested `model_id`、`base_url_stable_id`、不可逆 `secret_reference` 及相关版本；该 snapshot 与 AI annotations 作为离线 lineage/搜索内容保留。MCP **MUST NOT** 初始化或调用 provider，不读取可变 active 状态、workspace 数据库、Keyring 或 provider snapshot 作在线查询。release-bound snapshot 不算第二个 active profile，也不能用于 Studio 新写任务。
- API Key **MUST NOT** 写入 SQLite、普通配置文件、导出包、提示词、异常、日志、截图或前端响应；数据库和 release 只能保存不可逆的 `secret_reference`，WebUI 只能显示掩码。
- Studio 的 OpenAI 协议请求 **MUST** 使用图片输入、strict JSON Schema structured output、稳定错误分类、同一 configured/requested `model_id`、同一总重试预算、本地校验和最小必要披露。Responses 请求 **MUST** 使用 `store=false`，Chat Completions 请求 **MUST** 省略 `store`；响应 model echo 不作 equality gate，且不得据此声称已验证远端模型身份或 retention。成功响应仍须有 string `model`，缺失/非 string 必须 fail closed；不得自动改用另一协议或 model，warning/ack 不得绕过协议选择。MCP runtime 不执行 provider 请求。
...
- MCP 查询 **MUST** 先读取并解析 `current.json`，按 default/显式版本取得 pointer 的 `release_id` 与 `relative_path`，再只读取该 pointer 指定的 release。release 完整性、Schema/checksum/manifest/quality/index/PNG 预验证只属于 WebUI build/activation gate；MCP 运行时 **MUST NOT** 首次全量验证、逐请求验证 current/manifest/checksum/schema/file identity/hash、复算质量门或全量验证 PNG/index projection。MCP 只保留必要的相对路径安全检查、明显链接/reparse 拒绝、指定 index 打开和响应所需 PNG 按需读取；读取失败必须 fail closed 且不得回退其它 release。MCP 可在进程内按 `minecraft_version+release_id` 缓存 snapshot；下一请求观察到 pointer 指向变化时必须载入新 snapshot。MCP **MUST NOT** 初始化或调用任何 AI provider，不读取 Keyring 或 provider snapshot 作在线查询；除读取 release 和 `current.json` 外不得产生本地写入，尤其不得写数据库、文件、cache、logs 或 release 指针。
- MCP 的 stdout **MUST** 只输出 MCP 协议消息；日志、诊断和堆栈 **MUST** 输出 stderr，**MUST NOT** 污染 stdout，也不得写本地日志文件。
- MCP 返回的 `block_id`、状态、图片映射、release 元数据和本地搜索候选 **MUST** 来自 release；MCP 不初始化模型、不调用 provider、不做模型重排，不得伪装为已由 LLM 生成或重排。
- D-053（2026-08-20 owner-approved amendment）supersedes D-051 in full and supersedes D-052 only for provider/deadline/request-provider-reuse semantics：`search_blocks` 必须接收 `keywords` Unicode string array，删除 `query`、`context`、`query_spec`、server-side QuerySpec、natural-language/hard parser、visual rerank 和 provider runtime。只执行 pointer-resolved release 的 eligible/conditional 本地 FTS5 trigram 或 normalized LIKE 关键词召回、确定性排序、Top-24、limit/contact sheet；`data.query` 为按输入顺序 trim 后以单空格连接的 keywords，`hard_filters=[]`、`reranked_by_llm=false`、candidate `score_source=local`，空集为成功。前台/宿主 LLM 负责自然语言理解、关键词生成、澄清和零结果换词；MCP runtime 不初始化/调用 AI provider。query-spec/rerank Schema 与历史 provider/release fields 保留用于 Studio/历史 lineage，不属于 MCP runtime。D-052 的 build-time-only integrity、snapshot cache、to_thread/event-loop isolation 和 outputSchema `oneOf` 其余语义保留；不增加服务、依赖、Schema ID、SQL、migration、tool 或兼容层。
