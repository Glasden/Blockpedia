...
- **当前状态**：R0 契约冻结、R1 确定性导出和 R2 Index Studio/存储/任务均已有 Windows、静态与 fixture 证据；D-043 Gate 3/R1 P0 corrected-export closure 已由最终 pair evidence 验证，R1 P0 不再 open。D-040 的批次授权、D-041 的可扩展 aggregate authorization、D-042 的 `prompt.v2`/安全诊断、D-044 的 bounded concurrency 以及 D-045 的 targeted banner rerender/refresh 均已有 focused implementation 与 Windows runtime evidence。D-045 已将精确 32 个 banner targets 刷新到当前 R3 run，并完成 3 个新增 AI 批次；用户随后接受全部 1172 个 annotation reviews，并审核跳过剩余 15 个机器渲染失败项。真实 candidate-build gate 已通过，未激活 candidate `rel_357c6104fe8ccfc0f4b7823a68ccc84e` 已构建且根 `current.json` 不存在；既有 R4 Windows stdio/功能 evidence 已闭合。D-052 已于 2026-08-19 获 owner 批准并完成其既有实现/聚焦验收；2026-08-20 的 D-053 amendment 已完成契约、关键词输入、provider-free local search 实现和最小聚焦验收，证据独立于旧 R4/D-052 evidence。Linux CPython/Web、Linux MCP stdio、Linux wheel/ABI、Linux Java/runtime/exporter 和最终双平台复现统一 deferred 到 R5；正式支持平台与 Linux 基线不变，当前不声称 Linux 已通过。
...
依赖链为 `R0 → R1 → R2 → R3 → R4 → R5`。每一阶段只以该阶段已经定义的最小交付物和验收为退出条件；后续阶段的平台、运行时、数据和发布证据不得倒灌阻塞前一阶段。R0-R4 的既有阶段门均已退出；后续破坏性 MCP input contract amendment D-053 已完成实现与聚焦验收，R5 仍未完成。Linux 实际验证统一由 R5 承接，所有未完成项保持未勾选。
...
- [x] R4 fixture 使用 fresh-only `release-index.v2.sql`；历史 R3 `release-index.v1.sql` candidate 保持有效但不具备 activation 资格；future R4/R5 candidate 的 fresh-only index 边界由 build/activation gate 负责，D-052 后 MCP 运行时不检查 index format 或建立 v1 专用完整性错误路径。
...
- [x] （D-053 superseded historical implementation）实现确定性硬过滤/评分；本地 provider downgrade 保持确定性结果并返回 `reranked_by_llm=false`。该旧行为及其证据保留为历史事实，不再是当前 MCP runtime 契约。
- [x] 实现并提供 D-052 owner-approved 最小运行时契约：仅构建时验证、pointer snapshot/cache 切换、路径/reparse 安全边界、按需 index/PNG 读取、strict advertised `outputSchema` `oneOf` 及同步查询移出 event loop；锁定环境聚焦测试与最终 Oracle 复核均已通过。D-052 的 55/15/30 秒 provider budget、provider 请求资源复用和在线 QuerySpec/visual-rerank 语义仅作为历史证据保留，已由 D-053 supersede。
- [x] 实现并验证 D-053 owner-approved amendment：MCP 不初始化/调用 provider；`search_blocks` 使用 required `keywords` Unicode string array（1..16 项、每项 trim 后 1..64 chars、禁止空项/重复项），删除 `query`/`context`/`query_spec`，仅做 eligible/conditional 本地 FTS5 trigram 或 normalized LIKE 召回、确定性排序、Top-24 与 local score；输出 `data.query` 为 trim 后单空格连接的 keywords、`hard_filters=[]`、`reranked_by_llm=false`、candidate `score_source=local`，正常空集成功；宿主 LLM 负责自然语言理解和换词。独立 focused evidence 见本节 D-053 条目。
...
- [x] 四工具均只能读取 pointer-resolved release，不能写数据库、文件、日志、cache 或 `current.json`；协议测试记录执行前后 zero-write；release 完整性不由本项运行时复核。
- [x] 候选/图片结构化映射、状态和 release 查询通过 strict output、PNG ImageContent、one-based mapping 与 release/query core tests 验证；模型不得改变结构化事实；不将历史测试解释为 D-052 的 runtime integrity evidence。
...
- D-052 最小聚焦证据：CPython `3.14.7`、锁定 `mcp==2.0.0` 环境中，`python -m pytest -q -p no:cacheprovider tests/r4/test_mcp_stdio.py tests/r4/test_mcp_core.py tests/r4/test_release_projection.py tests/r3/test_provider_core.py` 在 remediation 后为 `114 passed`；最终仅修改 provider timeout 与 stdio thread-isolation 路径后，`python -m pytest -q -p no:cacheprovider tests/r3/test_provider_core.py tests/r4/test_mcp_stdio.py` 为 `84 passed`。相关 `py_compile` 与 `git diff --check` 通过，最终 Oracle 复核为 `PASS`。该 evidence 仅覆盖 D-052 既有语义，不覆盖 D-053 的 keywords/local-only/provider-free contract；D-053 使用下一条独立 evidence。未新增 evidence report、真实基数 fixture 或全量测试矩阵；owner 明确接受的离线篡改/损坏风险不被包装成等价安全。
- D-053 最小聚焦证据：Windows CPython `3.14.7`、锁定 `mcp==2.0.0` 环境中，`python -m pytest -q -p no:cacheprovider tests/r3/test_provider_core.py tests/r4` 输出 `101 passed in 13.96s`；`python -m py_compile src/blockpedia/mcp_query.py src/blockpedia/mcp_server.py src/blockpedia/provider.py src/blockpedia/activation.py` 与 `git diff --check` 均通过。覆盖 strict keywords input、旧字段拒绝、provider-free local results、短 Unicode 的 trigram/LIKE 路径、snapshot、`to_thread`、outputSchema parity、四工具 stdio 和 zero-write；未新增 evidence report、真实基数 fixture、依赖、服务、Schema ID、SQL 或测试矩阵。
