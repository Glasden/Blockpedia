...
项目所有者于 **2026-08-18** 明确批准为未来 R4/R5 candidate 增加 fresh-only `release-index.v2.sql`。现有 R3 Phase C `release-index.v1.sql` candidate 保持有效、不可变并继续作为 R3 evidence，但不具备 activation 资格；不得迁移、原地改写或将其伪装成 v2。v2 保留 v1 的 scalar/indexed columns 和 indexes，新增经过校验的 `blocks.record_json TEXT NOT NULL`、`states.record_json TEXT NOT NULL`、`visual_variants.record_json TEXT NOT NULL`、`visual_variants.feature_json TEXT NOT NULL`，并固定 `schema_meta.format_version=2`。R4/R5 future candidate 的 index 版本资格由 fresh build/activation gate 负责；D-052 后 MCP 运行时不检查或声称验证 `format_version`，也不为 v1 建立专门的运行时完整性错误路径。

**影响记录**：本项只增加未来 release index 的 fresh projection format，不改变现有 JSON Schema inventory、`workspace.v1.sql`、R3 evidence、工具集合、transport、服务、CLI、持久状态、release immutability 或 MCP 只读边界；不提供通用 migration，不重写旧 release。D-052 将 index format 的验证职责限定在 build/activation gate，不是对 v2 projection 本身的回退或迁移批准。
...
8. **边界保持**：MCP 仍只读取 current pointer 解析出的 release 路径、pointer 和必要的 release-bound secret reference；不读取 workspace/可变 active profile，不写数据库、文件、cache、logs、release 或 current。host spec 不得选择或改写 server-side provider profile、requested `model_id`、`base_url` 或 release snapshot；MCP 仍只提供四个工具、只使用 stdio，并保持精确版本解析和历史 release selector 禁止。release 完整性由 build/activation gate 负责，不由本条扩大运行时验证。
...
3. **输出广告与错误 parity**：四工具的 advertised `outputSchema` 必须是 strict `oneOf`，组合该工具既有成功 Schema 与既有 `mcp-error.v1`；不得新增 Schema ID、SQL、字段或 release manifest 字段。成功/工具错误仍分别保持 `isError=false/true`，`structuredContent` 与 TextContent 必须来自同一个已校验对象并保持 parity。协议层 input error 继续使用 JSON-RPC `-32602`，不能用工具错误替代。
4. **有界 provider 查询**：`search_blocks` 外层 hard deadline 为 55 秒；进入最后 5 秒后不得启动新的 provider 请求。QuerySpec stage 总预算为 15 秒（首次 10 秒、最多一次重试 5 秒），visual rerank stage 总预算为 30 秒（首次 20 秒、最多一次重试 10 秒）。每次实际 timeout 取 profile 配置、stage 剩余预算和 request 剩余时间的最小值；少于 1 秒不得发送。`auto` provider/超时/能力失败必须确定性本地降级并返回 warning；`required` 失败必须使用顶层 `RERANK_REQUIRED_UNAVAILABLE`。每个 logical provider request 最多两次尝试，不得 fallback、切换协议、profile 或 model。
5. **请求资源与执行上下文**：同一 `search_blocks` 请求复用一个 provider/client 和已经构造的 preview bytes；host-supplied QuerySpec 只省略 QuerySpec provider call，若需要 visual rerank 仍复用同一请求上下文。同步 provider/SQLite 查询工作必须移出 event loop。MCP 仍不写数据库、文件、cache、logs、release 或 current。
6. **明确不变项**：D-052 不增加服务、依赖、Schema ID、SQL、migration、MCP tool、transport 或 release manifest 字段；不改变 provider wire Schema、release 数据分层、四工具集合、stdio、版本 selector、local_only/auto/required 语义或 WebUI 构建/激活门。
7. **风险声明**：构建后的离线 release 篡改或损坏不保证在查询前发现；只有实际打开 index、执行 SQLite 查询或按需读取 PNG 时可能暴露。该风险由 owner 明确接受，文档、UI、错误和测试不得把这次检测削减包装成等价安全。
8. **最小 focused tests**：owner 只批准覆盖上述可观察行为的最小 focused tests：outputSchema strict `oneOf`、structured/Text parity 与 `isError` 分层；pointer/default/显式版本和下一请求切换；路径逃逸/明显链接或 reparse 拒绝；指定 index/按需 PNG 的读取失败 fail closed；55/15/30 秒预算、最后 5 秒禁止新 provider 请求、少于 1 秒不发送、最多两次尝试且无 fallback；provider/client/preview bytes 复用及同步工作不阻塞 event loop。不得新增真实基数 fixture、全量测试矩阵或新的 evidence report；没有新实现/测试证据时不得在路线图中勾选完成。

### D-035 impact proof

- **Local single-machine operation**：MCP 仍是本地 stdio 进程，只读取 current、pointer 指定 release、必要 Keyring 引用和按需响应资源；不增加守护进程、队列、远程协调器或其他运行服务。
- **Reproducibility and accepted security trade-off**：版本选择、pointer 的 release identity、严格输入、相对路径安全、确定性本地排序和 provider snapshot 选择保持可复核；但运行时不再重新证明 release 完整性，离线篡改可能直到实际读取失败才暴露。这是 owner 接受的安全检测削减，不是等价安全或完整性保证。
